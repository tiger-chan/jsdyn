...
test("test(box, circle)", (t) => {
	{
		...
		const shape1 = { verticies: aabb.vertices(box1) };
		let cs1 = { center: c1.center, support: gjk.bindSupportCircle(c1), shape: c1 };
		let cs2 = { center: box1.center, support: gjk.bindSupportPolygon(shape1), shape: shape1 };
...
test("distnace(circle, circle)", (t) => {
...

	{
		...
		t.true(distanceFound);
		approxEqual.exec(t, result.distance, 0.01, 0.001);
		approxEqualArray.exec(t, result.pointA, [1, 0]);
		approxEqualArray.exec(t, result.pointB, [1.01, 0]);
	}
...
test("distance(polygon, polygon)", (t) => {
...

	{
		...
		let cs1 = { center: center(poly1), support: gjk.bindSupportPolygon(poly1), shape: poly1 };
		let cs2 = { center: center(poly2), support: gjk.bindSupportPolygon(poly2), shape: poly2 };
		const state = gjk.createDistnaceState(cs1, cs2);
		const result = gjk.createDistanceResult();
		const distanceFound = gjk.distance(state, result);
		t.true(distanceFound);
